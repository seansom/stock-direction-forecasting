# This version of the direction forecasting mode recreates the model from the study by Song
# Title: Stock Trend Prediction: Based on Machine Learning Methods

# Model Type: LSTM (stacked LSTM layers with varying units and 0.2 dropout layers)
# Inputs: ['stock_return' (% based), 'rsi', 'adx', 'sar']
# Outputs: stock_return
# Loss: RMSE/MSE
# Optimizer: Adam
# Epochs: 50
# Batch Size: 80
...
from data_processing_Song import get_dataset, inverse_transform_data
...
def make_lstm_model(train_x, train_y, epochs=50):

    # The LSTM model to be used
    lstm_model = keras.models.Sequential([
        keras.layers.LSTM(units=20, input_shape=train_x.shape[1:], return_sequences=True),
        keras.layers.Dropout(0.2),
        keras.layers.LSTM(units=50, input_shape=train_x.shape[1:], return_sequences=True),
        keras.layers.Dropout(0.2),
        keras.layers.LSTM(units=100, input_shape=train_x.shape[1:], return_sequences=True),
    ...
    lstm_model.fit(train_x, train_y, batch_size=80, epochs=epochs, verbose=0, callbacks=[print_train_progress_callback])
...
def experiment(stock_ticker, time_steps, date_range=None, drop_col=None, test_on_val=False):
    ...
    lstm_model = make_lstm_model(train_x, train_y)
...
def main(stock_ticker):
    ...
    for i in range(repeats):
        ...
        perf, _, _, _, _ = experiment(stock_ticker, time_steps=5, date_range=None, drop_col=None)
