...
def make_lstm_model(input_shape):
    ...
    lstm_model = keras.models.Sequential([
        keras.layers.Bidirectional(keras.layers.LSTM(units=64, input_shape=input_shape, return_sequences=True, recurrent_dropout=0.2)),
        keras.layers.Bidirectional(keras.layers.LSTM(units=64, input_shape=input_shape, return_sequences=True, recurrent_dropout=0.2)),
        keras.layers.Bidirectional(keras.layers.LSTM(units=64, input_shape=input_shape, return_sequences=True, recurrent_dropout=0.2)),
...
def experiment(data):
    ...
    for index in range(data_len):
        ...
        lstm_model.fit(shuffled_train_x, shuffled_train_y, epochs=1024, validation_split=0.25, verbose=0, callbacks=[early_stopping_callback, print_train_progress_callback])
