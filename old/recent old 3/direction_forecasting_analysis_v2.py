...
from data_processing_analysis_v2 import get_dataset, inverse_transform_data
import time
import datetime
...
def get_hps(stock_ticker):
    if stock_ticker=='ALI':
        ...
        hps = {
            'units': 128, 'layers': 5, 'dropout': 0.3
        }
    elif stock_ticker=='PGOLD':
        hps = {
            'units': 256, 'layers': 3, 'dropout': 0.2
        }
...
def batch_analysis(stock_ticker, time_step):
    start_time = time.time()  
    #hps = get_hps(stock_ticker)
    hps = None
    repeats = 3
    drop_col = None
    #drop_col = ['divergence', 'rsi', 'ad', 'cmf', 'k_values', 'signal', 'p/e', 'sentiment', 'd_values', 'psei_returns', 'gdp', 'eps', 'adx', 'roe', 'real_interest_rate', 'atr']
    batch_size = 1200
    batch_dataset, col_names, grnd_truth = get_dataset(stock_ticker=stock_ticker, time_steps=time_step, drop_col=drop_col, batch_size=batch_size)
    batch_final_predictions = []
    batch_performances = []
    increase_list = []
    date_count = 0
    total_dates = len(grnd_truth)
    for item in batch_dataset:
        date_count = date_count + 1
        print(f"Current Date: {item['date']}; {date_count}/{total_dates}")
        performances = []
        increase = 0
        for i in range(repeats):
            print(f"Experiment {i + 1} / {repeats}")
            perf, _, _, prediction = experiment(item['scaler'], col_names, item['train_x'], item['train_y'], item['test_x'], item['test_y'], item['final_window'], hps)
            performances.append(perf)
            if prediction >= 0:
                increase = increase + 1    
        if increase > repeats-increase:
            final_prediction = 1
        else:
            final_prediction = 0
        increase_list.append(increase)
        mean_da = mean([perf['da'] for perf in performances])
        batch_final_predictions.append(final_prediction)
        batch_performances.append(mean_da)

    grnd_truth['prediction'] = batch_final_predictions
    grnd_truth['performance'] = batch_performances
    grnd_truth['increase'] = increase_list
    print(f"Time step: {time_step}")
    print(grnd_truth)
    gt1 = 0
    gt2 = 0
    for i in grnd_truth.index:
        if grnd_truth['prediction'][i] == grnd_truth['ground truth 1'][i]:
            gt1 = gt1 +1
        if grnd_truth['prediction'][i] == grnd_truth['ground truth 2'][i]:
            gt2 = gt2 +1
    print(f"gt1 accuracy: {gt1}/{total_dates}")
    print(f"gt2 accuracy: {gt2}/{total_dates}")
    total_time = str(datetime.timedelta(seconds=int(time.time() - start_time)))
    print(f"total time: {total_time}")
    #grnd_truth.to_csv(f"{stock_ticker}_{grnd_truth['date'][0]}-{grnd_truth['date'][total_dates-1]}.csv")
