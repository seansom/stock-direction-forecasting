...
def make_data_window(train, test, scaler, time_steps=1):
	...
	for i in range(train_len):
		...
		if (i + time_steps) < train_len:
			...
			train_y.append([1 if ((train[j, stock_returns_index] * scaler["Stock Returns"]["std"] + scaler['Stock Returns']['mean']) >= 0) else 0 for j in range(i + 1, i + time_steps + 1)])
			
	for i in range(test_len):
		
		if (i + time_steps) < test_len:
			test_x.append([test[j, :] for j in range(i, i + time_steps)])
			test_y.append([1 if ((test[j, stock_returns_index] * scaler["Stock Returns"]["std"] + scaler['Stock Returns']['mean']) >= 0) else 0 for j in range(i + 1, i + time_steps + 1)])
...
def make_lstm_hypermodel(hp, time_steps, features):
	...
	lstm_hypermodel.add(keras.layers.Dense(units=1, activation='sigmoid'))

	lstm_hypermodel.compile(loss='binary_crossentropy', optimizer='adam')
...
def experiment(stock_ticker, time_steps, repeats=2, overwrite=True):	
	...
	train_x, train_y, test_x, test_y = make_data_window(train, test, scaler, time_steps=time_steps)
