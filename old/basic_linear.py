...
def make_linear_model(train_x, train_y, epochs=100, batch_size=32):
	"""Builds, compiles, fits, and returns a linear model based on
	...
	Returns:
		Model: The built Keras linear model.
	"""	

	# The linear model to be used
	...
	linear_model.fit(train_x, train_y, epochs=epochs, batch_size=batch_size, validation_split=0.25,  verbose=0, callbacks=[early_stopping_callback, print_train_progress_callback])
...
def forecast_linear_model(model, test_x):
...
def get_linear_model_perf(predictions, actuals):
...
def experiment(stock_ticker, time_steps, epochs, batch_size):	
	...
	# create, compile, and fit a linear model
	linear_model = make_linear_model(train_x, train_y, epochs=epochs, batch_size=batch_size)

	# get the model predictions
	predictions = forecast_linear_model(linear_model, test_x)
	...
	perf = get_linear_model_perf(predictions, test_y)
