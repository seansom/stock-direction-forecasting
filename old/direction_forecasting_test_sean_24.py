...
def make_lstm_model(input_shape):
    ...
    lstm_model = keras.models.Sequential([
        keras.layers.LSTM(units=4, input_shape=input_shape, return_sequences=True, recurrent_dropout=0.2, dropout=0.6),
        keras.layers.LSTM(units=4, input_shape=input_shape, return_sequences=True, recurrent_dropout=0.2, dropout=0.6),
...
def experiment(data):
    ...
    for index in range(data_len):
        ...
        lstm_model.fit(shuffled_train_x, shuffled_train_y, epochs=1024, validation_split=0.025, verbose=1, callbacks=[early_stopping_callback])
