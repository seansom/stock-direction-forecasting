...
def log_preprocess_data(data):
    """Function which proceses raw values into usable data
    (i.e. Closing Stock Prices -> log of stock return)
    Args:
        data (DataFrame): The raw data read from a csv file.
    Returns:
        DataFrame: The processed dataset.
...
def make_lstm_model(train_x, train_y, hps):
    ...
    for _ in range(layers):
        lstm_model.add(keras.layers.LSTM(units=units, input_shape=(time_steps, features), return_sequences=True,
                                         recurrent_dropout=dropout))
    ...
    lstm_model.fit(train_x, train_y, epochs=100, validation_split=0.25, verbose=0,
                   callbacks=[early_stopping_callback, print_train_progress_callback])
...
def get_lstm_model_perf(predictions, actuals):
    ...
    return {"total_ups": total_ups, "total_downs": total_downs, "tp": tp, "tn": tn, "fp": fp, "fn": fn, "da": da,
            "uda": uda, "dda": dda}
