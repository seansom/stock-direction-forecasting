...
import datetime, requests, json, math, random, shelve, sys, os, re, pathlib
...
def train_test_split(data, time_steps):
    ...
    upward_target_indices = []
    downward_target_indices = []
    for i in target_indices:
        if data['log_return'][i] >= 0:
            upward_target_indices.append(i)
        else:
            downward_target_indices.append(i)

    upward_test_len = len(upward_target_indices) * 2 // 10
    downward_test_len = len(downward_target_indices) * 2 // 10

    train_targets = sorted(upward_target_indices[:-upward_test_len] + downward_target_indices[:-downward_test_len])
    test_targets = sorted(upward_target_indices[-upward_test_len:] + downward_target_indices[-downward_test_len:])
...
def data_processing(technical_data, fundamental_data, sentimental_data, time_steps, drop_col=None):
    ...
    return scaler, train, test, col_names, correlations, train_targets, test_targets, train_indices, test_indices
...
def get_dataset(stock_ticker, date_range=None, time_steps=1, drop_col=None):
    ...
    scaler, train, test, col_names, correlations, train_targets, test_targets, train_indices, test_indices = data_processing(technical_data, fundamental_data, sentimental_data, time_steps, drop_col)
...
def main():
    stock_ticker = 'ALI'
    scaler, col_names, correlations, train_x, train_y, test_x, test_y = get_dataset(stock_ticker, date_range=None, time_steps=1, drop_col=['psei_returns'])
