...
def get_technical_data(stock_ticker, date_range):
    ...
    EOD_indicators = [('rsi', 5), ('adx', 5), ('sar', -1)]
...
def train_test_split(data, time_steps):
    ...
    """	
    test_len = (data.shape[0] - time_steps) * 2 // 10
...
def get_dataset(stock_ticker, date_range=None, time_steps=1, drop_col=None):
    ...
    scaler, train, test, col_names = data_processing(technical_data, fundamental_data, sentimental_data, time_steps, drop_col)
...
def main():
    stock_ticker = 'MER'
    scaler, col_names, train_x, train_y, test_x, test_y = get_dataset(stock_ticker, date_range=None, time_steps=60, drop_col=['psei_returns'])
