# NOT OPTIMAL, TAKES A SHIT LOAD OF TIME TO FIND OPTIMAL HYPERPARAMS
...
def make_lstm_model(time_steps, features, units, layers, learning_rate, dropout):

	# The LSTM model to be used
	lstm_model = Sequential()

	for _ in range(layers):
		lstm_model.add(LSTM(units=units, input_shape=(time_steps, features), return_sequences=True, recurrent_dropout=dropout))
	lstm_model.add(Dense(units=1, activation="linear"))

	optimizer = Adam(learning_rate=learning_rate)

	lstm_model.compile(loss='mean_squared_error', optimizer=optimizer, metrics=['mean_squared_error'])
...
def grid_search(train, test, time_steps, features):

	def build_fn(units, layers, learning_rate, dropout):
		return make_lstm_model(time_steps, features, units, layers, learning_rate, dropout)

	train_x, train_y, _, _ = make_data_window(train, test, time_steps=time_steps)
	...
	grid_model = KerasRegressor(build_fn=build_fn, epochs=100, batch_size=32, validation_split=0.25, callbacks=[early_stopping_callback], verbose=0)

	grid = GridSearchCV(estimator=grid_model, param_grid=param_grid, n_jobs=-1, cv=5, verbose=3)
...
def experiment(stock_ticker, time_steps, epochs, batch_size):	
	...
	# perform grid search on lstm model to determine optimal hyperparameters
	grid_result = grid_search(train, test, time_steps=1, features=2)
	print(grid_result)
	sys.exit()
