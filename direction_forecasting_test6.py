...
from data_processing_old_4 import get_dataset, inverse_transform_data
...
def get_optimal_hps(train_x, train_y):
    ...
    tuner.search(train_x, train_y, validation_split=0.1111, callbacks=[early_stopping_callback])
...
def experiment(stock_ticker, time_steps, drop_col=None, test_on_val=False, hps=None):
    ...
    if test_on_val:
        test_len = train_x.shape[0] * 1111 // 10000
...
def main():
    ...
    hps = {'units': 64, 'layers': 3, 'dropout': 0.2, 'tuner/epochs': 12, 'tuner/initial_epoch': 0, 'tuner/bracket': 2, 'tuner/round': 0}
    hps = None
    ...
    # dropped features
    dropped_features = None#['cmf', 'atr', 'cci', 'slope', 'k_values', 'd_values', 'macd', 'signal', 'divergence', 'gdp', 'inflation', 'real_interest_rate', 'roe', 'eps', 'p/e', 'psei_returns', 'sentiment']
...
if __name__ == '__main__':
    ...
    for step in time_steps:
        curr_dropped_features = feature_selection(stock_ticker, step, repeats=10, hps=None)
