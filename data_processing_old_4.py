...
def scale_data(data, combined_data):
    ...
    for indicator in combined_data.columns:
        if (combined_data[f'{indicator}'] == 0).all():
            continue
        scale_factor = round(math.log10(combined_data[f'{indicator}'].abs().max()))
...
def train_test_split(data, time_steps):
    ...
    test_len = (len(data) - time_steps) * 1 // 10
    train, test = data[:-test_len], data[-test_len - time_steps:]

    return train, test


def transform_data(train, test, combined_data):
    ...
    scaler = scaler.fit(combined_data)
...
def data_processing(data_dict,  time_steps, drop_col=None):
    """Splits a dataset into training and testing samples.
    The train and test data are split with a ratio of 8:2.
    ...
    stock_index = data_dict['stock_index']
    combined_data = data_dict['combined_data']

    if drop_col is not None:
        combined_data.drop(columns=drop_col, inplace=True)

    train_sets = []
    test_sets = []

    for technical_data, fundamental_data, sentimental_data in zip(data_dict['technical_data_list'], data_dict['fundamental_data_list'], data_dict['sentimental_data_list']):
        ...
        #scale data
        scaled_data = scale_data(data, combined_data)
    ...
    combined_train_set = pd.concat(train_sets)

    transformed_train_sets = []
    transformed_test_sets = []

    for train, test in zip(train_sets, test_sets):
        #apply Yeo-Johnson Power Transfrom
        scaler, train, test, col_names = transform_data(train, test, combined_train_set)
        transformed_train_sets.append(train)
        transformed_test_sets.append(test)


    return scaler, transformed_train_sets, transformed_test_sets, stock_index, col_names
...
def get_dataset(stock_ticker, date_range=None, time_steps=1, drop_col=None):
    ...
    stocks = ['ALI', 'AP', 'BPI', 'JFC', 'MER', 'PGOLD', 'SM', 'TEL']
    ...
    for stock in stocks:
        ...
        # otherwise, get data from API and store it in database
        stock_database = shelve.open('stock_database')
        stock_database_key = f"{stock} {date_range}"
        ...
        if stock_database_key in stock_database:
            ...
            os.chdir('..')
            technical_data = get_technical_data(stock, date_range)
            fundamental_data = get_fundamental_data(stock, date_range)
            os.chdir('data')
        ...
        sentimental_data = get_sentimental_data(stock, date_range)

        technical_data_list.append(technical_data)
        fundamental_data_list.append(fundamental_data)
        sentimental_data_list.append(sentimental_data)


    combined_technical_data = pd.concat(technical_data_list)
    combined_technical_data = combined_technical_data.sort_index()
    combined_technical_data.reset_index(drop=True, inplace=True)

    combined_fundamental_data = pd.concat(fundamental_data_list)
    combined_fundamental_data = combined_fundamental_data.sort_index()
    combined_fundamental_data.reset_index(drop=True, inplace=True)

    combined_sentimental_data = pd.concat(sentimental_data_list)
    combined_sentimental_data = combined_sentimental_data.sort_index()
    combined_sentimental_data.reset_index(drop=True, inplace=True)

    combined_technical_data.drop(columns='date', inplace=True)
    combined_fundamental_data.drop(columns='date', inplace=True)
    combined_sentimental_data.drop(columns='date', inplace=True)
    
    combined_data = combined_technical_data.join(combined_fundamental_data)
    combined_data = combined_data.join(combined_sentimental_data)

    stock_index = stocks.index(stock_ticker)
    technical_data = technical_data_list[stock_index]
    fundamental_data = fundamental_data_list[stock_index]
    sentimental_data = sentimental_data_list[stock_index]
    ...
    data_dict = {
        ...
        'sentimental_data_list':sentimental_data_list,
        'combined_data':combined_data
    }

    scaler, train_sets, test_sets, stock_index, col_names = data_processing(data_dict, time_steps, drop_col)

    windowed_train_x_sets = []
    windowed_train_y_sets = []
    windowed_test_x_sets = []
    windowed_test_y_sets = []

    for train, test in zip(train_sets, test_sets):
        train_x, train_y, test_x, test_y = make_data_window(train, test, time_steps)

        windowed_train_x_sets.append(train_x)
        windowed_train_y_sets.append(train_y)
        windowed_test_x_sets.append(test_x)
        windowed_test_y_sets.append(test_y)
    ...
    for train_x, train_y in zip(windowed_train_x_sets, windowed_train_y_sets):
        
        validation_len = train_x.shape[0] * 1111 // 10000

        temp_train_x.append(train_x[:-validation_len, :, :])
        temp_train_y.append(train_y[:-validation_len, :])

        temp_val_x.append(train_x[-validation_len:, :, :])
        temp_val_y.append(train_y[-validation_len:, :])

    
    train_x = np.concatenate(temp_train_x)
    train_y = np.concatenate(temp_train_y)

    val_x = np.concatenate(temp_val_x)
    val_y = np.concatenate(temp_val_y)

    train_x = np.concatenate((train_x, val_x))
    train_y = np.concatenate((train_y, val_y))

    test_x = windowed_test_x_sets[stock_index]
    test_y = windowed_test_y_sets[stock_index]
...
def main():
    stock_ticker = 'AP'
    scaler, col_names, train_x, train_y, test_x, test_y = get_dataset(stock_ticker, date_range=('2017-04-13', '2022-04-13'), time_steps=1, drop_col=['ad', 'wr', 'cmf', 'atr', 'rsi', 'cci', 'adx', 'slope', 'k_values', 'd_values', 'macd', 'signal', 'divergence', 'gdp', 'inflation', 'real_interest_rate', 'roe', 'eps', 'p/e', 'psei_returns', 'sentiment'])
